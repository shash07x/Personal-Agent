...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".poison_profiles" / browser
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".poison_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using POISON profile")
                poison = str(Path.home() / ".poison_profiles" / "firefox_poison")
                Path(poison).mkdir(parents=True, exist_ok=True)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".poison_profiles" / "safari")
        ...
        poison_profile = str(Path.home() / ".poison_profiles" / self.browser_name)
        Path(poison_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with POISON profile: {poison_profile}")
    ...
    async def smart_click(self, description: str) -> str:
        ...
        for role in ("button", "link", "searchbox", "textbox", "menuitem", "tab"):
            try:
                ...
                pass
        for attempt in (
            lambda d=description: page.get_by_text(d, exact=False).first.click(timeout=5_000),
            lambda d=description: page.get_by_placeholder(d, exact=False).first.click(timeout=5_000),
            lambda d=description: page.locator(
                f'[alt*="{d}" i],[title*="{d}" i],'
                f'[aria-label*="{d}" i]'
